...
本课程使用目前 Spring Boot 最新版本2.0.3 RELEASE，
...
在实际项目中，常用的数据结构有类对象、List对象、Map对象，我们看一下默认的 jackson 框架对这三个常用的数据结构转成 json 后的格式如何。
...
> - `preHandle(……)` 方法：该方法的执行时机是，url匹配到controller之前。所以 `preHandle(……)` 方法可以决定是否将请求放行，这是通过返回值来决定的，返回 true 则放行，返回 false 则不会向后执行。 
>- `postHandle(……)` 方法：该方法的执行时机是，执行完了controller，在 DispatcherServlet 视图渲染之前。所以在这个方法中有个 **ModelAndView 参数**，可以在此做一些修改动作。  
> - `afterCompletion(……)` 方法：顾名思义，该方法是在整个请求处理完成后（包括视图渲染）执行，这时做一些**资源的清理**工作，这个方法只有在 `preHandle(……)` 被成功执行后并且返回 true 才会被执行。  
...
上文中已经介绍了拦截器的定义和配置，但是这样是否就没问题了呢？其实不然，如果使用上面这种配置的话，我们会发现一个缺陷，那就是**静态资源被拦截了**。可以在 resources/static/ 目录下放置一个图片资源或者 html 文件，然后启动项目直接访问，即可看到无法访问的现象。
