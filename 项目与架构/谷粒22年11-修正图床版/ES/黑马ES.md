ELK搜索高级课程 [https://www.bilibili.com/video/BV1if4y1P79q](https://www.bilibili.com/video/BV1if4y1P79q)

画的ES知识思维导图：[https://www.processon.com/view/link/614f2bebf346fb058d9dc7fc](https://www.processon.com/view/link/614f2bebf346fb058d9dc7fc)

本笔记获取方式：见本博客中谷粒商城文尾获取方式，邮件请备注【ES】

本文因为内容较为分撒，所以适合结合视频来学习。
...
![](https://i0.hdslb.com/bfs/album/0bc94c7f66d8549249823f9c7ec559f07aa76064.png)

随着elk的发展，又有新成员Beats、elastic cloud的加入，所以就形成了Elastic Stack。所以说，ELK是旧的称呼，Elastic Stack是新的名字。

![](https://i0.hdslb.com/bfs/album/2364b5640f1d5cbe040f717d268d9111ca310e9e.png))
...
![](https://i0.hdslb.com/bfs/album/f2d7a986ad7d7725f66326fc7ebbdd1804443d7c.png)
...
![](https://i0.hdslb.com/bfs/album/b9dba02dad0a0e222647f06c5d84f688e22c7409.png)

官网：https://www.elastic.co/cn/products/elasticsearch

![](https://i0.hdslb.com/bfs/album/b91e37ac35d939a1b9f610cc65b972bc5f8b7d61.png)
...
![](https://i0.hdslb.com/bfs/album/1145d01cc4dd75f304baf5b1129f0fba41891156.png)

![](https://i0.hdslb.com/bfs/album/302a6337bd707bc7b6c7d9297169a0d33bd273ae.png)

![](https://i0.hdslb.com/bfs/album/acd9f3e53eec81024f29e1f2848834d429ac15a7.png)
...
![](https://i0.hdslb.com/bfs/album/36311c14939537e452c1be635c1351c874373d1c.png)
...
![](https://i0.hdslb.com/bfs/album/3035b79a8cf398f4d45fdab7e7d05e0f0dd5b366.png)

​																	总览



![](https://i0.hdslb.com/bfs/album/d480082425e52eae9ee6285774bf4462254ed194.png)

​															Dev Tools界面

   ![](https://i0.hdslb.com/bfs/album/2becd6adb7a9c77b0c0c6af134c844ae0a260786.png)

​																监控集群界面

   ![](https://i0.hdslb.com/bfs/album/06a2d8bd287e366286220735ee5f6d7e7a951e3b.png)
...
get http://localhost:9200/
  ...
  ![](https://i0.hdslb.com/bfs/album/064441c2fd61995aee5d333407ea88428155456d.png)                                                

 

测试一下get方式查询集群状态http://localhost:9200/_cluster/health

   ![](https://i0.hdslb.com/bfs/album/0826ee8c9e015d3a7e3a99fb3b3c07ae998d7a30.png)
...
![](https://i0.hdslb.com/bfs/album/75dfb806024659ba00bac4d2d676c97b90a55776.png)
...
![](https://i0.hdslb.com/bfs/album/38cd75455540354be176eaa4f23bcf4e1dc1f269.png)
...
![](https://i0.hdslb.com/bfs/album/efd412d9f278af6ce7d2bd8319c44b897ecbdc99.png)
...
![](https://i0.hdslb.com/bfs/album/2c47d7553fcec96e0c68a979b4d9e90e54c922df.png)

下一步，再点击discover就可看到数据。

![](https://i0.hdslb.com/bfs/album/318b15d17f8b044672cc50db299709413be28795.png)

点击json还可以看到原始数据

![](https://i0.hdslb.com/bfs/album/7450919f26f0b0d0b2037c86780de1c232a9c0e2.png)

为方便查看索引中的数据，head可以如下操作

点击数据浏览，点击book索引。

![](https://i0.hdslb.com/bfs/album/58dd564f2f9f1bee4cc820f711661854c00ca32b.png)
...
- 扩容极限：节点数大于整体分片数，则必有空闲机器。超出扩容极限时，可以增加副本数，如设置副本数为2，总共3*3=9个分片。9台机器同时运行，存储和搜索性能更强。容错性更好。
-  容错性：只要一个索引的所有主分片在，集群就就可以运行。

## 8.6 容错机制 master选举，replica容错，数据恢复
...
```
nodel宕机，P0shard没有了，所有主分片不是全active，集群状态是red

容错第一步：
重新选举master节点。承担masteriä关功能。

容错第二步
1新master将丢失的主分片的摸个副本提升为主分片
2集群状态yew。
3现在少副本分片。

容错第三步．
1重启故障node
2新master会感知到新节，日入，将缺失的副本分片copy一份到新机器上
3copy的是增量数据。
4集群状态就是green．
```
...
场景：在程序中，架构师可以手动指定已有数据的一个属性为路由值，
...
涉及到以往数据的查询搜索，所以==一旦建立索引，主分片数不可变==。

## 9.2 文档的增删改内部机制

增删改可以看做update,都是对数据的改动。一个改动发送到es集群，经历以下四个步骤：
...
5、特殊情况：document如果还在建立索引(新增文档)过程中，可能只有primary shard有，任何一个replica shard都没有，此时可能会导致无法读取到document，但是document完成索引建立之后，primary shard和replica shard就都有了。
...
（1）不用将其转换为json对象，不会出现内存中的相同数据的拷贝，直接**按照换行符切割json**
...
GET /website/_search?q=post_date搜索列:2019-01-01     1条结果
...
例：2019-01-01，exact value，搜索的时候，必须输入2019-01-01，才能搜索出来

如果你输入一个01，是搜索不出来的，type=date时2，是精确匹配
...
- 3、token filter转换：lowercase，stop word，synonymom，dogs --> dog，liked --> like，Tom --> tom，a/the/an --> 干掉，mother --> mom，small --> little
...
![](https://i0.hdslb.com/bfs/album/2b65b73060d0783f3bc114fee4c062dc6a53702d.png)
...
是否在source之外存储，每个文档索引后会在 ES中保存一份原始文档，存放在"_source"中，一般情况下不需要设置store为true，因为在 _source中已经有一份原始文档了。
...
![](https://i0.hdslb.com/bfs/album/56752b3eea8dc5ac52b81af871742204a84c49ca.png)
...
{
  "title": "my article",
  "content": "this is my article", # 并没有这个字段
  "address": {
    "province": "guangdong",
    "city": "guangzhou"
  }
...
![](https://i0.hdslb.com/bfs/album/c5bd20255f95e5c4bf044ebb9f6fdd96f302092e.png)
...
PUT /my_index/_doc/2
{
  "title_en": "this is my first article"  # 这个可以被搜索到
}
```
...
- suffix.dic：放了一些后缀
- surname.dic：中国的姓氏
- stopword.dic：英文停用词。如a、the、and、at、but。一般，像停用词，会在分词的时候，直接被干掉，不会建立在倒排索引中
...
在文件IKAnalyzer.cfg.xml中有配置选项：ext_dict，所以我们创建mydict.dic。
...
设置超时时间的目的是不让ES查询太久，也许有更好的结果，但是不需要了
...
搜索原理：lclient发送一个搜索请求，会把请求打到所有的主分片上查询。
...
什么是deep paging：根据相关度评分倒排序，所以分页过深，协调节点会将大量数据聚合分析。
...
- 消耗CPU，coordinate node 要把传回来的数据进行排序，这个排序过程很消耗cpu。

所以：鉴于deep paging的性能问题，所有应尽量减少使用。

![](https://i0.hdslb.com/bfs/album/d18db042b1e956d1124e7503df750c43f01a0603.png)
...
es中 `_all`元数据。建立索引的时候，插入一条docunment，es会将所有的field值经行全量分词，把这些分词，放到`_all `field中。在搜索的时候，没有指定field，就在`_all`搜索。
...
- 方式1：fielddate:true。
- 方式2：将一个text field建立两次索引，一个分词，用来进行搜索；一个不分词，用来进行排序。
...
```json
{
  "_scroll_id" : "DXF1ZXJ5QW5kRmV0Y2gBAAAAAAAAMOkWTURBNDUtcjZTVUdKMFp5cXloVElOQQ==",  # 关键
  "took" : 3,
  "timed_out" : false,
  "_shards" : {
    "total" : 1,
...
![](https://i0.hdslb.com/bfs/album/58777b1e66209e90aca98929ed79e916cbc1e173.png)
...
![](https://i0.hdslb.com/bfs/album/953d72ebbd05819dc5fa1a39e91b9585857d6eb2.png)

![](https://i0.hdslb.com/bfs/album/e87c1c3a84453881380eaacfb43b35f166602116.png)
...
> 前面我们介绍过深度分页，那么实际上并不是将查到的所有数据区协调结点合并，而是先返回score和id，排序后再去获取doc内容
>
> ![](https://i0.hdslb.com/bfs/album/f80f66f8a2b9dfadad08ca1c08d7a4768d805314.png)
...
bouncing results问题，两个document排序field值相同；

不同的shard上，可能排序不同；

每次请求轮询路由到不同的replicashard上，每次页面上看到的搜索结果的排序都不一样。这就是bouncingresult的问题，这个问题出现最多的地方就是timestamp进行排序

比如当你使用一个timestap字段对结果进行排序，因为es中时间格式为。-%m-%d，那么同时间的数据会有很多。es如果不做任何设置，将会按round-robined的方式从primarfinreplica里取了再排序，这样结果就不能保证每次都一样的。毕竟primary有的relica里不一定有，尤其是
在不停往es里存放数据的情况，如果有两份文档拥有相同的timestampo因为搜索请求是以一种循环（Round一robin）的方式被可用的分片拷贝进行处理的，因此这两份文档的返回顺序可能因为处理的分片不一样而不同，比如主分片处理的顺序和副本分片处理的顺序就可能不一样。这就是结果跳跃问题：每次用户刷新页面都会发现结果的顺序不一样。

![](https://i0.hdslb.com/bfs/album/61d496f7a3f39bcafadeb6bc278989dbc7730bdd.png)
...
已经讲解过原理了，主要就是限定在一定时间内，**将获取到的部分数据直接返回**，避免查询耗时过长
...
- metric：count(*)，对每个user_id bucket中所有的数据，计算一个数量。还有`avg()`，sum()，max()，min()
...
GET /tvs/_search 
{
  ...
  "aggs": {
    "group_by_sold_date": {
      "date_histogram": {
        "field": "sold_date", # date类型
        "interval": "quarter", # 每隔季度
        "format": "yyyy-MM-dd",
        "min_doc_count": 0,
        "extended_bounds": {
          "min": "2019-01-01",
...
![](https://i0.hdslb.com/bfs/album/3a5ca34cbf46408d37e1997ed8fc6a62a18c6582.png)

数据源如：日志文件

输出：ES、文件、kafka、redis
...
通过上面这个组合匹配模式，我们**将输入的内容分成了五个部分**，即五个字段，将输入内容分割为不同的数据字段，这对于日后解析和查询日志数据非常有用，这正是使用grok的目的。
...
![](https://i0.hdslb.com/bfs/album/befd724650b6575a62a8261e4e2776ce94f027bb.png)
